...
| 配置 | 订阅地址（右端一个节点状态灯）+ 自动更新订阅周期 + OpenCode 请求头开关 + 「保存并应用」，以及「重启内核」「清零统计」「手动重置」 |
| 运行日志 | 内核和网关的实时日志 |
...
**自动更新订阅。** 单位为小时，默认每 1 小时更新一次；填 `0` 可关闭。每次自动更新完成后都会自动测一遍延迟。修改周期只重排下一次更新时间，不重启 mihomo。

**OpenCode 请求头。** 默认关闭，出站仍是原来的 `User-Agent: node`。开启后会透传客户端给出的 OpenCode request/session/project/client 等请求头，缺失的 request/session ID 为当前客户端请求补默认值；同一请求换节点重试会复用同一组 ID，下一次客户端请求会生成新 ID。只改这个开关不会刷新订阅、测速、重启 mihomo、重写 mihomo 配置或重排自动更新时间。
