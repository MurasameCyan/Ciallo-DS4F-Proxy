...
面板「接入」卡里有两颗按钮:「复制 OpenAI 协议」给你带 `/v1` 的地址、「复制 Anthropic 协议」给你不带 `/v1` 的裸地址(Claude Code 那类客户端自己拼 `/v1/messages`)。两者都取你当前的访问地址,反代/端口映射后面也对。
...
| 概览 | 一张卡里 2×2 四格:左上 Token 消耗(输入·输出·推理·缓存读·缓存写)、右上 运行时长(最后请求)、左下 客户端请求总数(成功·失败·成功率)、右下 调用统计 |
| 接入 | 接入地址(「复制 OpenAI 协议」/「复制 Anthropic 协议」两颗按钮)、API Key(掩码显示,「重置」/「复制」)、可用模型 |
...
**「调用统计」和「调用日志」不是一个口径。** 概览右下那格按模型统计**客户端请求**里成功的次数,降序排,一次都没成功过的不列(列一行 0 只是占位)。调用日志那张表记的是**上游调用**:同一个客户端请求换节点重试三次会在日志里留三行,但在调用统计里只算一次。排序键是成功数而不是请求数 —— 一个模型每次都 429 却排在榜首没有意义。
