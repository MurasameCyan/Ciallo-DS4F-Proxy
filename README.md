...
**429 轮换。** 免费额度按出口 IP 计。撞到 429 就把当前节点冷却 90 秒、换下一个重发,对客户端是透明的。最多换 6 次节点 —— 48 个全试一遍没意义,同一个机场的出口经常是同一段 IP。
...
| 统计 | 客户端请求总数(成功·失败)、成功率、Token 消耗(输入·输出·推理·缓存)、运行时长(最后请求·主用模型) |
| 接入 | Base URL、API Key(掩码显示,「重置」/「复制」)、可用模型 |
| 配置 | 订阅地址(右端一个节点状态灯)+ OpenCode 身份头实验开关 + 「保存并应用」,以及「重启内核」「清零统计」「手动重置」 |
| 运行日志 | 内核和网关的实时日志 |
| 节点池 | 按实测延迟排序,从上往下就是网关接下来会用的顺序。每行带延迟数字,当前节点标出来,冷却中的显示剩余秒数,测不通的划掉垫在最底下 |
| 节点统计 | 按真实上游尝试展示每个节点的成功率、429、超时、上游错误、Token 与缓存命中率 |
...
状态灯和那三个按钮都在「配置」卡里,和订阅地址挨着 —— 看一眼状态然后动手是一回事,分在页头和卡片两个地方要来回扫视。灯只剩一个(节点数):网关端口在接入卡的 Base URL 上已经有了,内核版本看一眼就够、不会变,真会动的只有节点数。内核挂了的时候借它报「内核未运行」—— 只显示「无节点」的话,看不出是订阅没填还是内核死了,这两件事的处置完全不同。
...
**OpenCode 身份头(实验)。** 默认关闭,出站仍是原来的 `User-Agent: node`。开启后会透传客户端给出的 OpenCode request/session/project/client 等身份头,缺失的 request/session ID 为当前客户端请求补默认值；同一请求换节点重试会复用同一组 ID,下一次客户端请求会生成新 ID。只改这个开关不会刷新订阅、测速、重启 mihomo 或重写 mihomo 配置。
...
| 文件 | 内容 |
| --- | --- |
| `config.json` | 订阅地址、API Key、端口、`opencodeIdentityHeaders` 开关。**含机场 token**,别往外发 |
...
| `/api/config` | GET · POST | 读取/保存订阅地址与 `opencodeIdentityHeaders`;明确提交订阅时当场应用,只切身份头不碰内核。`port` 只读,提交了也忽略 |
