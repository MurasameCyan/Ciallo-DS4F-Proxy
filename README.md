...
**稳定 session 标识。** 网关优先把 Claude Code 的 `x-claude-code-session-id` 映射成 `x-opencode-session`;也兼容客户端显式提供的其它会话 ID,都没有时才按第一条 user 内容的 SHA-256 生成。优先级最高的仍是客户端直接给的 `x-opencode-session`。同一对话增长后 ID 不变,换节点重试也复用同一个 ID。它只用于让上游有机会保持会话和 prompt cache,不增加额度,也不提供并发隔离。
