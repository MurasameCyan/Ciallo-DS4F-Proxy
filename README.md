...
| `NODE_TEST_URL` | | 测速探针地址,默认 `http://www.gstatic.com/generate_204`。国内直连不到 gstatic 无所谓 —— 这条请求是走节点发出去的 |
| `NODE_TEST_TIMEOUT_MS` | | 单节点测速超时,默认 `5000`。超时算不可用 |
...
**测速排序 + 剔除死节点。** 订阅解析完自动测一遍延迟(并发 6),快的排前面,轮换就按这个顺序取。测不通的直接不进候选表 —— 不然一个已经下线的节点每次轮换都要先浪费一次超时。两条保险:全部节点都测不通时**不剔除任何一个**(那更像是探针地址本身不可达,不能让网关自己瘫掉),没测过的节点也不算死。面板上每个节点显示实测延迟,「测速」按钮可以随时重测。
